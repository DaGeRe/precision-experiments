...
do
	...
	do
		...
		do
			...
			methodCompressedCount=$(cat $file/$testcase/$testmethod/*_method | egrep -v "[ ]*\([0-9]*\)[ ]*$" | wc -l)
			methodCompressedLines=$(cd $file/$testcase/$testmethod/; \
				...
				| egrep -v "[ ]*\([0-9]*\)[ ]*$" \
				| wc -l)
			depth=$(cat $file/$testcase/$testmethod/*_method | awk '{match($0, /^ */); print RLENGTH;}' | sort | tail -n 1)
